...
// The following implementation of the Par type is provided by Manning as a solution to the multiple implementation
...
object Examples {
  def sum(
    ...
    if (ints.size <= 1)
      ...
      val (l, r) =
        ints.splitAt(ints.length / 2) // Divide the sequence in half using the `splitAt` function.
